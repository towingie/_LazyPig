...
function LazyPig_Dismount()
	...
	local tooltipfind2 = "Increases movement speed by (.+)%%"
	while GetPlayerBuff(counter) >= 0 do
		...
		if desc then
			...
			_, _, speed2 = string.find(desc, tooltipfind2) 
			if speed or speed2 then
...
function LazyPig_IsWolfForm()
	local i;
	local max = GetNumShapeshiftForms();
	for i = 1 , max do
		local _, name, isActive = GetShapeshiftFormInfo(i);
		if(isActive and LazyPig_PlayerClass("Shaman","player") and (name == "Ghost Wolf")) then
		    return true
		end
	end
	return false
...
function LazyPig_CheckSalvation()
	if(LPCONFIG.SALVA == 1 or LPCONFIG.SALVA == 2 and (LazyPig_IsShieldEquipped() and LazyPig_PlayerClass("Warrior", "player") or LazyPig_IsBearForm() or LazyPig_IsWolfForm())) then
